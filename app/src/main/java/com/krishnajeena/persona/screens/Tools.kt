...
fun VideoPlayer(videoUrl: String) {
        ...
        // Observe lifecycle changes to pause video when in background
        DisposableEffect(lifecycleOwner) {
                val observer = LifecycleEventObserver { _, event ->
                        when (event) {
                                Lifecycle.Event.ON_PAUSE -> exoPlayer.playWhenReady = false // Pause when in background
                                Lifecycle.Event.ON_RESUME -> exoPlayer.playWhenReady = true // Resume if needed
                                else -> {}
                        }
                }
